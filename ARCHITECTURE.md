...
Other distributions, including tested GRUB-based installations of EndeavourOS and CatchyOS, may embed the prefix directly in the EFI executable and have no EFI-side `grub.cfg`. Both layouts are handled by `grub-install`; BootPrep does not manually interpret or rewrite either representation.
