...
BootPrep is compatible with Debian, Ubuntu, EndeavourOS, and many related derivatives that meet these requirements. Version 2.0.0 has been exercised directly on Debian, Kubuntu, and EndeavourOS. Distribution packaging and Snapper integration still vary, so testing with a verified recovery path is recommended before deployment on another distribution.
...
Version 2.0.0 has been tested through fresh installation, v1 migration where applicable, clean upgrade, explicit activation, native Snapper rollback, and reboot on Debian, Kubuntu, and EndeavourOS systems.
